...
public class UserController {
    ...
    @PostMapping("/addUser")
    public ResponseEntity<AwsUserDTO> addUser(@RequestBody AwsUserDTO userDTO){
        AwsUserDTO savedUser =  userService.addUser(userDTO);
        return new ResponseEntity<>(savedUser, HttpStatus.CREATED);
    }

    @GetMapping("/fetchUserById/{userId}")
    public ResponseEntity<AwsUser> fetchUserDetailsById(@PathVariable Long userId){
